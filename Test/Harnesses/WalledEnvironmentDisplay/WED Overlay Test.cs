...
namespace Bardez.Projects.InfinityPlus1.Test.Harnesses.WalledEnvironmentDisplay
{
    ...
    public class WedOverlayTest : FileTesterBase
    {
        #region Constants
        ...
        {
            this.FilePaths = ConfigurationHandlerMulti.GetSettingValues(WedOverlayTest.configKey);
        }
        ...
        protected override void TestCase(Object sender, TestEventArgs testArgs)
        {
            using (FileStream stream = new FileStream(testArgs.Path, FileMode.Open, FileAccess.Read))
            {
                ...
                ReusableIO.SeekIfAble(stream, headerTemp.OffsetOverlays);

                this.overlay = new Overlay();
                this.DoPostMessage(new MessageEventArgs("Reading WED overlay...", "Reading", testArgs.Path));
                this.overlay.Read(stream);
                this.DoPostMessage(new MessageEventArgs(this.overlay.ToString(), "Output", testArgs.Path));
            }

            using (FileStream dest = new FileStream(testArgs.Path + ".overlay.rewrite", FileMode.Create, FileAccess.Write))
                this.TestWrite(dest);
        }
        ...
        {
            this.overlay.Write(destination);
        }
        #endregion
    }
}
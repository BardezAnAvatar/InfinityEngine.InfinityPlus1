...
namespace Bardez.Projects.InfinityPlus1.Test.Harnesses.WalledEnvironmentDisplay
{
    ...
    public class WedPolygonHeaderTest : FileTesterBase
    {
        #region Constants
        ...
        protected PolygonHeader header { get; set; }
        #endregion


        #region Construction
        /// <summary>Default constructor</summary>
        public WedPolygonHeaderTest()
        {
        ...
        {
            this.FilePaths = ConfigurationHandlerMulti.GetSettingValues(WedPolygonHeaderTest.configKey);
        }
        ...
        protected override void TestCase(Object sender, TestEventArgs testArgs)
        {
            using (FileStream stream = new FileStream(testArgs.Path, FileMode.Open, FileAccess.Read))
            {
                ...
                ReusableIO.SeekIfAble(stream, headerTemp.OffsetPolygonHeader);

                this.header = new PolygonHeader();
                this.DoPostMessage(new MessageEventArgs("Reading WED polygon header...", "Reading", testArgs.Path));
                this.header.Read(stream);
                this.DoPostMessage(new MessageEventArgs(this.header.ToString(), "Output", testArgs.Path));
            }

            using (FileStream dest = new FileStream(testArgs.Path + ".polyheader.rewrite", FileMode.Create, FileAccess.Write))
                this.TestWrite(dest);
        }

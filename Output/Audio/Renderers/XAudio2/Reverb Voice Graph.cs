...
{
    /// <summary>Represents the Voice graph for a renderer that has a direct path and a reverberation path</summary>
    public class ReverbVoiceGraph : IVoiceGraph
    {
        #region Fields
        ...
        /// <summary>XAudio2 reverb-path Submix intermediate voice</summary>
        protected SubmixVoice SubmixVoice;
        ...
        /// </remarks>
        protected IEffectParameter XAudio2ReverbParameter;
        ...
        /// <summary>The last <see cref="FilterParameter" /> set from Source Voice to Mastering Voice</summary>
        protected FilterParameter ReverbPathFilterParameter;
        #endregion
        ...
        /// <param name="reverb">Reverberation effect to apply to the Source Voice's samples</param>
        public ReverbVoiceGraph(WaveFormatEx sourceAudioFormat, String renderingDeviceName, AudioRenderStyle behavior, ReverbSettings reverb)
        {
            ...
            this.SubmixVoice = this.XAudio2.CreateSubmixVoice(this.AudioFormat.NumberChannels, this.AudioFormat.SamplesPerSec);
            this.SourceVoice = this.XAudio2.CreateSourceVoice(this.AudioFormat);
            this.SubmixVoice.SetOutputVoices(new VoiceSendDescriptor[] { new VoiceSendDescriptor(0U, this.MasteringVoice) });   //render from submix to mastering
            this.SourceVoice.SetOutputVoices(new VoiceSendDescriptor[] { new VoiceSendDescriptor(0U, this.SubmixVoice), new VoiceSendDescriptor(0U, this.MasteringVoice) });   //render from source to both submix and to mastering

            //set up reverb effect
            EnvironmentalReverbEffect envReverbEffect = new EnvironmentalReverbEffect();
            this.XAudio2ReverbParameter = new EnvironmentalReverbParameters(reverb);
            List<EffectDescriptor> effectChain = new List<EffectDescriptor>() { new EffectDescriptor(envReverbEffect, true, this.AudioFormat.NumberChannels) };
            this.SubmixVoice.SetEffectParameters(0U, this.XAudio2ReverbParameter, 0U);  //actually set the reverberation
        }
        #endregion

...
{
    /// <summary>Represents a Baldur's Gate 2 save game</summary>
    public class GameBaldur2 : IInfinityFormat
    {
        #region Fields
        ...
        public List<JournalEntry_v2> JournalEntries { get; set; }

        /// <summary>Collection of stored locations</summary>
        public List<StoredCoordinate> StoredLocations { get; set; }

        /// <summary>Collection of stored Pocket Plane locations</summary>
        public List<StoredCoordinate> StoredPocketPlaneLocations { get; set; }
        ...
        public virtual void Initialize()
        {
            ...
            this.JournalEntries = new List<JournalEntry_v2>();
            this.StoredLocations = new List<StoredCoordinate>();
            this.StoredPocketPlaneLocations = new List<StoredCoordinate>();
        ...
        public virtual void ReadBody(Stream input)
        {
            ...
            {
                JournalEntry_v2 entry = new JournalEntry_v2();
                entry.Read(input);
                this.JournalEntries.Add(entry);
            }

            //stored locations
            ReusableIO.SeekIfAble(input, this.Header.StoredLocatonsOffset);
            for (Int32 index = 0; index < this.Header.StoredLocatonsCount; ++index)
            {
                StoredCoordinate location = new StoredCoordinate();
                location.Read(input);
                this.StoredLocations.Add(location);
            }

            //stored Pocket Plane locations
            ReusableIO.SeekIfAble(input, this.Header.PocketPlaneStoredLocatonsOffset);
            for (Int32 index = 0; index < this.Header.PocketPlaneStoredLocatonsCount; ++index)
            {
                StoredCoordinate location = new StoredCoordinate();
                location.Read(input);
                this.StoredPocketPlaneLocations.Add(location);
            }
        ...
        public virtual void Write(Stream output)
        {
            ...
            foreach (JournalEntry_v2 entry in this.JournalEntries)
                entry.Write(output);

            //stored locations
            ReusableIO.SeekIfAble(output, this.Header.StoredLocatonsOffset);
            if (output.Length < this.Header.StoredLocatonsOffset)
                output.SetLength(this.Header.StoredLocatonsOffset);
            foreach (StoredCoordinate location in this.StoredLocations)
                location.Write(output);

            //stored Pocket Plane locations
            ReusableIO.SeekIfAble(output, this.Header.PocketPlaneStoredLocatonsOffset);
            if (output.Length < this.Header.PocketPlaneStoredLocatonsOffset)
                output.SetLength(this.Header.PocketPlaneStoredLocatonsOffset);
            foreach (StoredCoordinate location in this.StoredPocketPlaneLocations)
                location.Write(output);
        ...
        protected List<Tuple<Int64, Int64>> CollectOffsets()
        {
            ...
            //stored locations
            offsetCollection.Add(new Tuple<Int64, Int64>(this.Header.StoredLocatonsOffset, (this.Header.StoredLocatonsCount * StoredCoordinate.StructSize)));

            //pocket plane stored locations
            offsetCollection.Add(new Tuple<Int64, Int64>(this.Header.PocketPlaneStoredLocatonsOffset, (this.Header.PocketPlaneStoredLocatonsCount * StoredCoordinate.StructSize)));
        ...
        protected virtual void ResetOffsets()
        {
            ...
            //stored locations
            this.Header.StoredLocatonsOffset = offset;
            offset += (Convert.ToInt32(this.Header.StoredLocatonsCount) * StoredCoordinate.StructSize);

            //pocket plane stored locations
            this.Header.PocketPlaneStoredLocatonsOffset = offset;
            offset += (Convert.ToInt32(this.Header.PocketPlaneStoredLocatonsCount) * StoredCoordinate.StructSize);
        ...
        public override String ToString()
        {
            ...
            builder.AppendLine("Baldur Game version 2.0");
            ...
            builder.Append(this.GenerateStordLocationString(this.StoredLocations));
            builder.Append(this.GenerateStordLocationString(this.StoredPocketPlaneLocations));
        ...
        /// <summary>Generates a human-readable console output describing the variables</summary>
        /// <returns>A multi-line String</returns>
        ...
        /// <summary>Generates a human-readable console output describing StoredCoordinates</summary>
        /// <returns>A multi-line String</returns>
        protected String GenerateStordLocationString(IList<StoredCoordinate> collection)
        {
            StringBuilder sb = new StringBuilder();
